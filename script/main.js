...
<p>{event_content}
    <div>
        <div class="more-link">
            <a href="{event_url}">More...</a>
        </div>
        <div class="blog-publisher">{event_timings} @ <span class="blog-venue">{event_venue}</span></div>
    </div>
</p>
</article>`;
...
const doSearchForPostsProjectsBySearchTerm = (searchKeyword) => {
    ...
    if (posts.length > 0) {
        posts.forEach((post) => {
            const postData = POST_TEMPLATE.replace('{blog_content}', post.content)
                .replace('{blog_author}', post.source)
                .replace('{type}', 'News')
                .replace('{blog_title}', post.title).replace('{blog_url}', post.url)
            searchData += postData;
        });
    ...
    if (events.length > 0) {
        events.forEach((eventItem) => {
            const eventData = EVENTS_TEMPLATE.replace('{event_content}', eventItem.content)
                .replace('{event_timings}', eventItem.timings)
                .replace('{event_title}', eventItem.title)
                .replace('{event_url}', eventItem.url)
                .replace('{type}', 'Event')
                .replace('{event_venue}', eventItem.venue);
...
const loadBlogPosts = () => {
    $.get(BLOG_POST_URL, function (response) {
        ...
        const blogPosts = response.map((article) => POST_TEMPLATE.replace('{blog_content}', article.content)
            .replace('{blog_author}', article.source)
            .replace('{blog_title}', article.title).replace('{blog_url}', article.url));
...
const loadEvents = () => {
    $.get(EVENTS_URL, function (response) {
        ...
        const events = response.map((eventItem) => EVENTS_TEMPLATE.replace('{event_content}', eventItem.content)
            .replace('{event_timings}', eventItem.timings)
            .replace('{event_title}', eventItem.title)
            .replace('{event_url}', eventItem.url)
            .replace('{event_venue}', eventItem.venue));
